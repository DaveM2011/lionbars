(function($) {
  jQuery.fn.extend({removeCSS: function(cssName) {return this.each(function() {return jQuery(this).attr('style', jQuery.grep(jQuery(this).attr('style').split(";"), function(curCssName) {if (curCssName.toUpperCase().indexOf(cssName.toUpperCase() + ':') <= 0) return curCssName;}).join(";"));});}});
  
  jQuery.fn.hasScrollBar = function() {return this.get(0).scrollHeight > this.height();};
  jQuery.fn.lionbars_fn = function(options) {
    // Flags
    var timeout, loaded = false,
        HDragging = false,
        VDragging = false,
        activeScroll = 0,
        activeWrap = 0,
        eventX, eventY, mouseX, mouseY, currentRatio, initPos, scrollValue, hideTimeoutSet = false,
        vEventFired = false,
        hEventFired = false;
    // Initialization
    var element = $(this),
    ...
    // Main Loop
    var lion = {};
    lion.scrollToBottom = function() {
      if (needScrollbars(element) && !$(element).hasClass('nolionbars') && loaded) {
        lion.Update();
        target = element;
        ...
        if (!options.animate) b.scrollTop(m);
        else b.animate({
          scrollTop: m
        });
      }
    }
    lion.setSide = function(side) {
      options.side = side;
      switch (side) {
      case 'left':
        element.find('.lb-v-scrollbar').removeCSS('right').css('left', 0);
        break;
      case 'right':
        element.find('.lb-v-scrollbar').removeCSS('left').css('right', 0);
        break;
      }
    }
    lion.init = function() {
      if (needScrollbars(element) && !$(element).hasClass('nolionbars')) {
    ...
    function setEvents(elem) {
      ...
      if ((addVScroll || addHScroll) && options.autohide) {
  ...
  $.fn.lionbars = function(options) {
    ...
    return this.each(function() {
      var d = jQuery(this).lionbars_fn(options);
      jQuery.data(this, 'lion', d);
    });
  };
})(jQuery);